<template lang="html">
  ...
  >
    <profile-header
      ...
      :profile-id="updatedProfileId || profile.id"
    />
    <br />
    <profile-description
      :is-viewer="viewer"
  ...
  methods: {
    async loadFavoriteProfiles() {
      ...
      const favorites = await this.$store
        .dispatch("favorites/loadFavorites", this.updatedProfileId)
        .then(res => res)
        .catch(err => {
          this.error = err;
          return this.error;
    ...
    async checkProfile() {
      ...
      const result = await this.$store
        .dispatch("auth/findAccountById", {
          accountId: this.updatedProfileId,
          viewer: this.viewer
        })
        .catch(err => {
          ...
          this.error = {
            message:
              "Désolé, il semblerait que nous avons un problème pour afficher ce profil"
          };
          return setTimeout(() => {
            this.$router.go(-1);
          }, 1000);
