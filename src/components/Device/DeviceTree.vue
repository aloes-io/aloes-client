...
export default {
    ...
    nodes() {
      if (this.graphNodes) {
        return select(`#device-tree-${this.deviceId}`)
          ...
          .attr('r', d => (d.data.size ? this.nodeSize * d.data.size : this.nodeSize))
    ...
    images() {
      if (this.graphNodes) {
        return select(`#device-tree-${this.deviceId}`)
          ...
          .enter()
          .append('image')
          .attr('xlink:href', this.setImageUrl)
          .attr('crossOrigin', 'anonymous')
          .attr('x', d =>
            d.data.appKey ? `${(-1 * this.nodeSize) / 2}` : `${(-1 * this.nodeSize) / 2 / 2}`,
          )
          .attr('y', d =>
            d.data.appKey ? `${(-1 * this.nodeSize) / 2}` : `${(-1 * this.nodeSize) / 2 / 2}`,
          )
          .attr('width', d => (d.data.appKey ? this.nodeSize : this.nodeSize / 2))
          .attr('height', d => (d.data.appKey ? this.nodeSize : this.nodeSize / 2))
          .style('cursor', 'pointer')
  ...
  methods: {
    ...
    nodeFill(d) {
      if (d.data.group === 0) return this.colors.yellow;
      if (d.data.group === 1) return this.colors.blue;
      if (d.data.group === 2) return this.colors.green;
      return this.colors.grey;
    },

    linkDistance(d) {
      let ratio = 0.5;
      if (d.target.data.group === 1) ratio = 0.7;
      if (d.target.data.group === 2) ratio = 0.3;
