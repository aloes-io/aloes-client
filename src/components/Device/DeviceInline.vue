  ...
  >
    <b-row>
      <b-col cols="2" sm="4" md="4" lg="4" xl="3">
        <img
          :src="updatedDevice.icons[0]"
          class="device-inline-icon"
          @click="goToDevice"
        />
        <!--  <b-button
      ...
      <b-col cols="10" sm="8" md="8" lg="8" xl="9">
        <b-row class="device-inline-row">
          <b-col class="profile-props" sm="12">
            <h6 class="device-inline-name">
              {{ updatedDevice.name }} - {{ updatedDevice.type }}
            </h6>
        ...
        <b-row>
          <b-col class="device-inline-status" cols="12">
            ...
            <div
              v-else-if="updatedDevice.status"
              class="device-inline-status-on"
            >
  ...
  mounted() {
    EventBus.$on("deviceSelected", device => {
      if (
        device &&
        device !== null &&
        device.id.toString() === this.updatedDevice.id.toString()
      ) {
