...
export default {
  ...
  methods: {
    async initObjectComposition() {
      ...
      this.objectCompositionSimulation = forceSimulation(
        this.objectCompositionGraph.nodes
      )
        .alphaDecay(0.005)
        .alpha(0.2)
        ...
        .force(
          "charge",
          forceManyBody(this.objectCompositionGraph.nodes).strength(-800)
        )
        .force(
          "center",
    ...
    dragstarted(d) {
      if (!event.active)
        this.objectCompositionSimulation.alphaTarget(0.3).restart();
      d.fx = d.x;
      d.fy = d.y;
    },
