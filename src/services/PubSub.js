...
PubSub.unSubscribeWhere = async (client, options) => {
  if (options && client && Storage) {
    const name = `/${options.accountId}/${options.collectionName}/${
      options.method
    }`;
...
PubSub.subscribeToInstanceUpdate = async (
  client,
  collectionName,
  userId,
  modelId
...
PubSub.setListeners = async (client, token) => {
  ...
  if (client && client !== null && token && token !== null) {
    await PubSub.subscribeToCollectionCreation(
      client,
      "Notification",
      token.userId
    );
        ...
        switch (protocol.collectionName) {
          ...
          default:
            logger.publish(
              4,
              "PubSub",
              `on${protocol.collectionName}Created`,
              message
            );
        }
        break;
        ...
        switch (protocol.collectionName) {
          ...
          default:
            logger.publish(
              4,
              "PubSub",
              `on${protocol.collectionName}Deleted`,
              message
            );
        }
        break;
        ...
        switch (protocol.collectionName) {
          ...
          default:
            logger.publish(
              4,
              "PubSub",
              `on${protocol.collectionName}Updated`,
              message
            );
        }
        break;
